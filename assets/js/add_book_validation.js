...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function validateForm() {
    ...
    const maxAuthorLength = 80;

    const errors = [];

    if (title.trim() === "" || title.length > maxTitleLength) {
      errors.push(
        `Invalid book title. Maximum length is ${maxTitleLength} characters.`
      );
    }

    if (author.trim() === "" || author.length > maxAuthorLength) {
      errors.push(
        `Invalid author name. Maximum length is ${maxAuthorLength} characters.`
      );
    }
